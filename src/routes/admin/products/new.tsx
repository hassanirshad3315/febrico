...
import { safeDbMutation, saveStoredProduct } from "@/lib/studio-persistence";
import { toast } from "sonner";

export const Route = createFileRoute("/admin/products/new")({
  head: () => ({
    meta: [{ title: "Create New Product — FABRICO Admin" }],
  }),
...
function AdminNewProductPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    fit: "Regular fit",
    season: "All Season",
    care: "Dry clean recommended. Iron on low heat on the reverse.",
    short_description: "",
    description: "",
    sizes: ["XS", "S", "M", "L", "XL"],
    colors: "Ivory",
    tags: "embroidered, luxury",
    images: "/images/look-1.jpg\n/images/look-5.jpg",
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        image_alts: imagesArray.map((_, i) => `${formData.name} view ${i + 1}`),
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5 sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Ivory Embroidered Raw Silk 3 Piece"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="Optional reduction"
                  className="w-full bg-muted/40 border border-border p-2.5 outline-none focus:border-foreground"
                />
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-1.5">
                <label className="font-semibold text-foreground">Fabric Type</label>
                <input
                  type="text"
                  value={formData.fabric}
                  onChange={(e) => setFormData({ ...formData, fabric: e.target.value })}
                  placeholder="Raw Silk, Lawn, Velvet..."
                  className="w-full bg-muted/40 border border-border p-2.5 outline-none focus:border-foreground"
                />
              </div>

              <div className="space-y-1.5">
                <label className="font-semibold text-foreground">Fit Silhouette</label>
                <input
                  type="text"
                  value={formData.fit}
                  onChange={(e) => setFormData({ ...formData, fit: e.target.value })}
                  placeholder="Regular fit, Flowing cut..."
                  className="w-full bg-muted/40 border border-border p-2.5 outline-none focus:border-foreground"
                />
              </div>
              ...
              <div className="space-y-1.5 sm:col-span-3">
                <label className="font-semibold text-foreground">Care Instructions</label>
                <input
                  type="text"
                  value={formData.care}
          ...
          <div className="pt-4 border-t border-border flex justify-end gap-3">
            ...
            >
              ...
              <span>Publish Product to Studio</span>
            </button>
          </div>
        </form>
      </div>
